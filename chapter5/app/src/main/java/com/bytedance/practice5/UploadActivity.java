...
public class UploadActivity extends AppCompatActivity {
    ...
    private String defaultCoverPaths;
    ...
    private void initDefaultCovers() {
        //TODO 8
        // 获取assets文件夹的图片
        // 生成默认图片路径并放置
        AssetManager am = this.getAssets();
        String[] pciPaths = null;
        try {
            pciPaths = am.list("");  // ""获取所有,填入目录获取该目录下所有资源
            for (int i = 0; i < pciPaths.length; i++) {
                if (pciPaths[i].endsWith(".jpg") || pciPaths[i].endsWith(".png")) {  // 根据图片特征找出图片
                    defaultCoverPaths = pciPaths[i];
                    break;
                }
            }
            Bitmap bitmap = getAssetsBitmap(this, defaultCoverPaths);
    ...
    private void submit() {
        Map<String, MultipartBody.Part> UploadParts = getUploadData();
